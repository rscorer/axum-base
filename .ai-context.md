...
- **Tailwind CSS**: Utility-first CSS framework with standalone CLI
  ...
  - Builds: `build.rs` builds Tailwind CSS during development and production
...
- **Build Scripts**: `build.rs` handles Tailwind CSS compilation during development and production builds
- **Template Integration**: CSS linked in `templates/base.html` via `/static/style.css`

#### Development Workflow
1. Start Axum server with `cargo watch run` in main terminal 
2. Tailwind monitors templates and rebuilds CSS when classes change 
3. Browser receives updated CSS automatically (no manual refresh needed)

#### Production Workflow
1. Generates minified CSS with unused styles purged
2. Ensures optimal performance and minimal file size for production deployment
3. Includes content-based hashing for cache busting in production environments
