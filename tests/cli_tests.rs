...
async fn test_create_user_cli_non_interactive() {
    ...
    assert!(
        ...
        "create_user command should succeed. stderr: {}",
...
async fn test_create_user_cli_invalid_args() {
    ...
    assert!(
        !output.status.success(),
        "create_user should fail with invalid args"
    );
...
async fn test_create_user_cli_empty_email() {
    ...
    assert!(
        !output.status.success(),
        "create_user should fail with empty email"
    );
...
async fn test_create_user_requires_password() {
    ...
    let user = test_db
        .create_test_user("haspassword", "has@example.com", "validpassword123")
        .await;
...
async fn test_set_password_cli_success() {
    ...
    let user = test_db
        .create_test_user("pwuser", "pwuser@example.com", "oldpassword123")
        .await;
    let user_id = user.id;
    ...
    assert!(
        output.status.success(),
        "set_password command should succeed. stderr: {}",
        String::from_utf8_lossy(&output.stderr)
    );
...
async fn test_set_password_cli_invalid_user_id() {
    ...
    assert!(
        !output.status.success(),
        "set_password should fail with invalid user ID"
    );
...
async fn test_set_password_cli_invalid_args() {
    ...
    assert!(
        !output.status.success(),
        "set_password should fail with invalid args"
    );
...
async fn test_set_password_cli_non_numeric_user_id() {
    ...
    assert!(
        !output.status.success(),
        "set_password should fail with non-numeric user ID"
    );
...
async fn test_set_password_cli_short_password() {
    ...
    let user = test_db
        .create_test_user("shortpw", "shortpw@example.com", "validpassword")
        .await;
    ...
    assert!(
        !output.status.success(),
        "set_password should fail with short password"
    );
...
async fn test_create_user_cli_duplicate_username() {
    ...
    // Create first user
    let _user1 = test_db
        .create_test_user(&unique_username, "first@example.com", "password123")
        .await;
    ...
    assert!(
        !output.status.success(),
        "create_user should fail with duplicate username"
    );
